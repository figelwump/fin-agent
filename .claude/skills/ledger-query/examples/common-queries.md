...
```bash
fin-query saved recent_transactions --limit 15 --format csv
```
...
```bash
fin-query saved category_summary --param month=2025-09 --limit 50 --format csv
```
...
```bash
fin-query saved transactions_month --param month=2025-09 --limit 200 --format csv
```
...
```bash
fin-query saved uncategorized --limit 50 --format csv
```
...
```bash
fin-query saved merchant_patterns --param pattern=%AMAZON% --limit 20 --format csv
```
...
```bash
fin-query saved merchant_search --param pattern=%YouTube TV% --limit 20 --format csv
```
...
```bash
fin-query saved recent_imports --limit 10 --format csv
```
...
```bash
fin-query saved categories --param category=%Dining% --limit 50 --format csv
```
...
```bash
fin-query sql "SELECT date, merchant, amount FROM transactions WHERE amount < -200 ORDER BY date DESC" --limit 10 --format csv
```
