...
```bash
fin-analyze category-breakdown --month 2025-10 --format csv
```
...
```bash
fin-analyze merchant-frequency --month 2025-09 --category "Food & Dining" --subcategory "Restaurants" --format csv
```

Has my spending changed compared to last month?
```bash
fin-analyze category-breakdown --month 2025-10 --compare --format csv
```

Show my dining spending over the last 6 months
```bash
fin-analyze category-timeline --period 6m --category "Food & Dining" --interval month --format csv
```

Summarize all-time spending
```bash
fin-analyze category-breakdown --period all --format csv
```
